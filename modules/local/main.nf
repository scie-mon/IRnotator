...
workflow {
    ...
    if (has_proteins && has_genome && has_gff) {
        throw new IllegalArgumentException('Invalid input: do not provide proteins_faa, genome_fasta, and annot_gff together.')
    }
    ...
    if (has_proteins && !has_genome && !has_gff) {
        ...
        normalize_res = NORMALIZE_GENOME_GFF(
            ...
            Channel.fromPath(params.annot_gff, checkIfExists: true),
            normalizer_script_ch
        )
    }
    else if (has_proteins && !has_genome && has_gff) {
        normalize_res = NORMALIZE_PROTEINS_GFF(
            Channel.fromPath(params.proteins_faa, checkIfExists: true),
            Channel.fromPath(params.annot_gff, checkIfExists: true),
            normalizer_script_ch
        )
    }
    else {
        throw new IllegalArgumentException('Invalid input: provide exactly one valid combination: (1) --proteins_faa; (2) --genome_fasta plus --annot_gff; or (3) --proteins_faa plus --annot_gff.')
    }
    ...
    hmm_res = HMMSEARCH_IR(hmms_ch.combine(normalized_proteins_ch))
    ...
    initial_deeptmhmm_res.summary.view { summary_tsv ->
        def counts = summary_tsv.toFile().readLines().drop(1).collectEntries { line ->
            def fields = line.split('\t', 2)
            [(fields[0]): fields[1]]
        }
        "DeepTMHMM candidates: ${counts.hmm_candidates}; salvaged: ${counts.salvaged}; queued for new prediction: ${counts.queued_for_prediction}"
    }

    def generated_results_ch
    if (params.run_deeptmhmm) {
        if (!params.deeptmhmm_dir) {
            throw new IllegalArgumentException('Missing --deeptmhmm_dir while --run_deeptmhmm is enabled.')
        }
        deeptmhmm_res = DEEPTMHMM_TOPOLOGY(
            initial_deeptmhmm_res.unresolved_fasta.flatten(),
            Channel.value(file(params.deeptmhmm_dir))
        )
        generated_results_ch = deeptmhmm_res.results.map { sequence_id, result_dir -> result_dir }.collect()
    }
    else {
    ...
    final_deeptmhmm_results_ch = final_deeptmhmm_res.result_dirs.flatten().map { result_dir -> tuple(result_dir.baseName, result_dir) }
    ...
    seed_res = SEED_DECISIONS(parsed_res.features, hardcoded_res.scores, params.manual_review)

    if (params.manual_review) {
        prepare_res = PREPARE_MANUAL_REVIEW(
            final_deeptmhmm_results_ch.collect(), hardcoded_res.scores, sequence_registry_ch,
            file("${projectDir}/assets/reviewer/topology-reviewer.html")
        )
        review_tsv_path = file("${params.outdir}/manual_review/review.tsv").toAbsolutePath().toString()
        gate_res = GATE_REVIEW_TSV(review_tsv_path, prepare_res.review_dir)
        decisions_ch = APPLY_MANUAL_DECISIONS(seed_res.decisions, gate_res.review_tsv).decisions
    }
    else {
        decisions_ch = seed_res.decisions
    }
    passed_res = EXTRACT_PASSED_FASTA(decisions_ch, hmm_hit_faa_ch)
}
