...
**Contents**  
- [Installation and prerequisites](#anchor-1 "#anchor-1")  
- [Quick start via the controller](#anchor-2 "#anchor-2")  
- [Direct Nextflow usage](#anchor-3 "#anchor-3")  
- [Input routes and DeepTMHMM backends](#anchor-4 "#anchor-4")  
- [Manual-review cycle](#anchor-5 "#anchor-5")  
- [Parameters](#anchor-6 "#anchor-6")  
- [Outputs, reruns, and result salvage](#anchor-7 "#anchor-7")  
- [HPC and container configuration](#anchor-8 "#anchor-8")  
...
*[!NOTE]*  
 *  
 Local DeepTMHMM installation requires a separately obtained licensed distribution. IRnotator can instead reuse externally generated DeepTMHMM results; see *[ *Outputs, reruns, and result salvage* *.*](#anchor-7 "#anchor-7")  
**Obtain IRnotator**  
...
1. Obtain the DeepTMHMM standalone package through the [official request form.](https://forms.cloud.microsoft/Pages/ResponsePage.aspx?id=rURVGECW5U69G3ew50KVeHoV9Bi7sdNBnLmAyRfuhiJUM1lRRVY0MExIMEU1V0UxN1ZDVkFFUzlVQi4u "https://forms.cloud.microsoft/Pages/ResponsePage.aspx?id=rURVGECW5U69G3ew50KVeHoV9Bi7sdNBnLmAyRfuhiJUM1lRRVY0MExIMEU1V0UxN1ZDVkFFUzlVQi4u")  
2. Extract the received archive into a persistent location readable from every execution node:  
3. mkdir -p /path/to/software  
 unzip DeepTMHMM-v1.0.zip -d /path/to/software  
   
4. Confirm that the extracted directory contains either predict.py directly, or deeptmhmm/predict.py. These are the two layouts supported by IRnotator's local runner.  
5. Configure the path in IRnotator/nextflow.config or a site-specific Nextflow config:  
params {  
     deeptmhmm_mode = 'container'  
     deeptmhmm_dir  = '/absolute/path/to/DeepTMHMM'  
 }  
   
6. Supply a config at launch time (if not using IRnotator/nextflow.config):  
...
nextflow run main.nf -profile docker -c irnotator.local.config \  
   --genome_fasta /path/to/genome.fna \  
   --annot_gff '/path/to/annotations/*.gff3' \  
   --hmm_dir /path/to/ir_hmms --outdir results -resume  
   
...
   
With the controller, rerun the same command without -resume. With direct Nextflow, rerun with -resume. Retain the work directory to reuse cache entries. Unresolved topology sequences are written to <outdir>/deeptmhmm/unresolved_deeptmhmm.faa.  
