...
set -euo pipefail
    ...
    cp merged.unfiltered.gff3 manual_review/reviewer_context.gff3
...
for internal_id, item in parse_context(REVIEW_DIR / "reviewer_context.gff3").items():
    ...
    subprocess.run([
        "python3", "render_gene_cds.py", "merged.unfiltered.gff3", str(gene_out),
...
PY
    ...
    if [[ 0 -gt 0 ]]; then
        cp .no_applied_review.tsv manual_review/review.applied.rev0.tsv
        rm -f "mulit_GFF_test3/manual_review/review.tsv"
    fi
