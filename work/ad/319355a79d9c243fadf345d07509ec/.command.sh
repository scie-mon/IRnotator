...
set -euo pipefail
    ...
    cp merged.rev0.gff3 manual_review/reviewer_context.gff3
...
for internal_id, item in parse_context(REVIEW_DIR / "reviewer_context.gff3").items():
    ...
    subprocess.run([
        "python3", "render_gene_cds.py", "merged.rev0.gff3", str(gene_out),
...
PY
    ...
    if [[ 1 -gt 0 ]]; then
        cp review.tsv manual_review/review.applied.rev1.tsv
        rm -f "wrapper_test/manual_review/review.tsv"
    fi
