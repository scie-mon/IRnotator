...
set -euo pipefail
    ...
    cp merged.rev2.gff3 manual_review/reviewer_context.gff3
...
for internal_id, item in parse_context(REVIEW_DIR / "reviewer_context.gff3").items():
    ...
    subprocess.run([
        "python3", "render_gene_cds.py", "merged.rev2.gff3", str(gene_out),
...
PY
    ...
    if [[ 3 -gt 0 ]]; then
        cp review.tsv manual_review/review.applied.rev3.tsv
        rm -f "wrapper_test2/manual_review/review.tsv"
    fi
